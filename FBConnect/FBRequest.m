    ...
    if ([dataDictionary count] > 0) {
        for (id key in dataDictionary) {
            ...
            if ([dataParam isKindOfClass:[UIImage class]]) {
                ...
                [self utfAppendBody:body
                               data:[[NSString alloc] initWithString:@"Content-Type: image/png\r\n\r\n"]];
                ...
                [self utfAppendBody:body
                               data:[NSString stringWithFormat:
                                     @"Content-Disposition: form-data; filename=\"%@\"\r\n", key]];
                [self utfAppendBody:body
                               data:[[NSString alloc] initWithString:@"Content-Type: content/unknown\r\n\r\n"]];
...
- (id)parseJsonResponse:(NSData*)data error:(NSError**)error {
    ...
    //
    //
    //    NSDictionary *arr = [dic objectForKey:@"summary"];
    //    NSLog(@"arr:%@",arr);
    
    //    NSString *str=[NSString stringWithFormat:@"%@",[arr objectForKey:@"unseen_count"]];
    //    facebookNOTIFICATION =str;
    //    NSLog(@"facebookNOTIFICATION:%@",facebookNOTIFICATION);
    
    
    SBJSON1 *jsonParser = [[SBJSON1 new] autorelease];
