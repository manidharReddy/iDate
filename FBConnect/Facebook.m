...
- (void) openUrl:(NSString *)url
    ...
    if ([self isSessionValid]) {
       ...
       // NSLog(@"fb Accesstoken:%@",self.accessToken);
...
- (void) authorize:(NSString*)application_id
   ...
   // NSLog(@"appId:%@",application_id);
    NSLog(@"permisType:%@",permissions);
...
- (void)logout:(id<FBSessionDelegate>)delegate {
    ...
    [self requestWithMethodName:@"auth.expireSession"
                    ...
                    andDelegate:self];
    //NSLog(@"facebook_token exp:%@",params);
    [params release];
    ...
    //NSLog(@"facebook_cookie:%@",facebookCookies);
...
- (void)fbDialogLogin:(NSString *)token expirationDate:(NSDate *)expirationDate {
    NSLog(@"bddialogLogin:%@and:%@",token,expirationDate);
